...
class AnnDataModel:
    ...
    def get_cells_metadata_by_id(self, cell_ids, max_return=100):
        if self.adata is None:
            raise ValueError("No data loaded.")
            
        # Filter existing cell IDs
        existing_cell_ids = [cell_id for cell_id in cell_ids if cell_id in self.adata.obs_names]
        
        if not existing_cell_ids:
            return "No matching cells found."
            
        # Randomly sample if too many cells
        if len(existing_cell_ids) > max_return:
            existing_cell_ids = random.sample(existing_cell_ids, max_return)
            
        # Get metadata for selected cells
        metadata_df = self.adata[existing_cell_ids].obs
        
        # Convert to comma-separated format
        csv_data = metadata_df.to_csv(index=True)
