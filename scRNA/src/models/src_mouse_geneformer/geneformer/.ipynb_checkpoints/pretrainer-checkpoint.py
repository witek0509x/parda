...
# github(https://github.com/huggingface/transformers/blob/v4.39.0/src/transformers/data/data_collator.py)と同じものを追加
...
class GeneformerPreCollator(SpecialTokensMixin):
    def __init__(self, *args, **kwargs) -> None:
        ...
        super().__init__(mask_token = "<mask>", pad_token = "<pad>", sep_token = "<sep>")
        ...
        # ]
        pretext_task = kwargs.get("pretext_task", None)
        if pretext_task == "NSP" :
            self.model_input_names = ["input_ids", "labels", "token_type_ids"] #labels, token_type_ids を追加 
        elif pretext_task == "MLM" :
            self.model_input_names = ["input_ids"]
        elif pretext_task == "BERT" :
            self.model_input_names = ["input_ids", "labels", "next_sentence_label", "token_type_ids"] #labels, token_type_ids を追加 
        else :
            print("Do not specify Pre Text Task or must do debug to kwards!")
            sys.exit(1)
...
# NextSentencePrediction用のDataCollatorを追加
class DataCollatorForNextSentencePrediction(DataCollatorForLanguageModeling):
...
# BertPreTraining用のDataCollatorを追加（多分完成：未実行）
...
class GeneformerPretrainer(Trainer): # 事前学習をしている
    def __init__(self, pretext_task="MLM", *args, **kwargs):
        data_collator = kwargs.get("data_collator",None)
        token_dictionary = kwargs.pop("token_dictionary")

        if data_collator is None:
            precollator = GeneformerPreCollator(token_dictionary=token_dictionary, pretext_task = pretext_task)

            # # Data Collator Functions
            # # データの加工（mask処理をしたりpaddingしたり）とバッチデータセット作成
            # # バッチデータセット作成するためのSamplerは，CustomDistributedLengthGroupedSamplerか，torch.utils.data.RandomSampler
            if pretext_task == "MLM" :
                data_collator = DataCollatorForLanguageModeling( # Masked Language Modeling用のDataCollator
                    tokenizer=precollator, mlm=True, mlm_probability=0.15
                )
            elif pretext_task == "NSP" :
                data_collator = DataCollatorForNextSentencePrediction( # Next Sentence Prediction用のDataCollator
                    tokenizer=precollator
                )
            elif pretext_task == "BERT" :
                data_collator = DataCollatorForBertPreTraining( # BertPreTraining用のDataCollator
                    tokenizer=precollator, mlm=True, mlm_probability=0.15
                )
            else :
                print("Do not specify Pre Text Task or must do debug to kwards!")
...
class CustomDistributedLengthGroupedSampler(DistributedLengthGroupedSampler):
    ...
    def __init__(
        ...
        seed: int = 0,
        drop_last: bool = False, # (default: False)
...
def get_length_grouped_indices(
    ...
    if mega_batch_mult is None:
        ...
        mega_batch_mult = min(len(lengths) // (batch_size * 4), 1000) # mega_batch_mult = min(int(22226957 / 12*4 = 463061), 1000)
        # Just in case, for tiny datasets
        if mega_batch_mult == 0:
            mega_batch_mult = 1

    # We need to use torch for the random part as a distributed sampler will set the random seed for torch.
    indices = torch.randperm(len(lengths), generator=generator) # 0から22226957までのランダムな並びを生成
    megabatch_size = mega_batch_mult * batch_size # 1000 * 12 = 12000
    megabatches = [
        indices[i : i + megabatch_size].tolist()
        for i in range(0, len(lengths), megabatch_size) # range(0, 22226957, 12000)→0から12000飛び出22226957まで抽出するから1852個選ばれる（はず）
    ]
    megabatches = [ # megabatchesリストの中のindexが指す次元数を大きい順位ソート
    ...
    megabatch_maximums = [lengths[megabatch[0]] for megabatch in megabatches] # 各バッチに属するデータの最大次元数を取得
    max_idx = torch.argmax(torch.tensor(megabatch_maximums)).item() # 最大次元数の中での最大次元数のindexを取得
    # Switch to put the longest element in first position
    megabatches[0][0], megabatches[max_idx][0] = ( # 最大次元数のデータの位置を先頭のデータの位置とチェンジ
        megabatches[max_idx][0],
        megabatches[0][0],
    )

    return [item for sublist in megabatches for item in sublist] # 2次元リストmegabatchesを1次元リストに変換している（megabatchesの要素である最大次元数順のindexリストの中身一個一個を取り出し1次元にしている）
